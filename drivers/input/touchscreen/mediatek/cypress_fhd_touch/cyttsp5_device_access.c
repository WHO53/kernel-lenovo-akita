	...
	struct configuration *configuration)
{
	...
	if (tx_num != dad->si->sensing_conf_data.tx_num) {
		dev_err(dev, "%s: TX number mismatch! tx_num:%d,dad->si->sensing_conf_data.tx_num:%d,rx_num:%d,dad->si->sensing_conf_data.rx_num:%d\n", __func__,tx_num,dad->si->sensing_conf_data.tx_num,rx_num,dad->si->sensing_conf_data.rx_num);
		ret = -EINVAL;
	}
...
int cyttsp5_cmcp_test(void)
{
	struct device *dev = g_cd->dev;
	struct cyttsp5_device_access_data *dad
		= cyttsp5_get_device_access_data(dev);
	struct cmcp_data *cmcp_info = dad->cmcp_info;
	struct result *result = dad->result;
...
start_testing:
	printk("%s: cyttsp5 Start Cm/Cp test!\n", __func__);
	result->cm_test_pass = false;
	result->cp_test_pass = false;
	/*stop watchdog*/
	rc = cmd->request_stop_wd(dev);
	if (rc)
		dev_err(dev, "stop watchdog failed");
...
no_builtin:
	...
	if (result->test_summary) {
		printk("%s: Finish Cm/Cp test! All Test Passed\n",
				__func__);
		// index = snprintf(buf, CY_MAX_PRBUF_SIZE,
		// 		"Status 1\n");
		index = 0; //0-success !0 failure
	} else {
		printk("%s: Finish Cm/Cp test! Range Check Failure\n",
				__func__);
		// index = snprintf(buf, CY_MAX_PRBUF_SIZE,
		// 		"Status 6\n");
		index = 1;
...
u32 cmcp_return_offset_of_new_case(const char *bufPtr, u32 first_time, const char* pFileEnd)
{
