...
static void dump_charger_name(enum charger_type type)
{
	...
	case APPLE_0_5A_CHARGER:
		...
		break;
	case POGO_CHARGER:
		pr_debug("charger type: %d, Pogo Charger\n", type);
		break;
	default:
...
void mtk_pmic_enable_chr_type_det(bool en)
{
	...
	if (en) {
		if (is_meta_mode()) {
			...
			pr_debug("check pogo status: %d \n", pogo_get_state());
			if (g_chr_type == NONSTANDARD_CHARGER) {
				if (0x02 == pogo_get_state()) {
					g_chr_type = POGO_CHARGER;
					pr_debug("force chr type for pogo \n");
				}
...
#endif
				while(detect_times--){
					pr_debug("NONSTANDARD CHARGER detect:%d\n",detect_times);
					g_chr_type = hw_charging_get_charger_type();
			...
			if (g_chr_type == NONSTANDARD_CHARGER) {
				...
				if (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
					...
					if (0x4 == i_ACA_type) {
						g_chr_type = STANDARD_CHARGER;
						pr_debug("force chr type for ac\n");
					}
				}
			}
...
void chrdet_int_handler(void)
{
	...
	if (!pmic_get_register_value(PMIC_RGS_CHRDET)) {
		...
		if (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
		    || boot_mode == LOW_POWER_OFF_CHARGING_BOOT) {
			pr_debug("[chrdet_int_handler] Unplug Charger/USB\n");
