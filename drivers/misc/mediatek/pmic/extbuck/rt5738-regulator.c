	...
	struct rt5738_regulator_info *info, struct device *dev)
{
	...
	if (!np) {
		pr_debug("%s cant find node (0x%02x)\n",
	...
	if (ret >= 0) {
		...
		pr_debug("%s use chip default vsel_pin(0)\n", __func__);
		info->pin_sel = 0;
		info->id = 0;
	}
					...
					const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		pr_debug("%s parse dt (0x%02x) fail\n", __func__, i2c->addr);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		pr_debug("%s rt5738 regmap init fail\n", __func__);
		return -EINVAL;
	}
	...
	if (rt5738_read_byte(info->i2c, RT5738_REG_MONITOR, &ret) >= 0)
		g_is_rt5738_exist = 1;
	pr_debug("i2c_addr=%d ret=%d g_is_rt5738_exist=%d\n"
				, i2c->addr
				, ret
				, g_is_rt5738_exist);
	return 0;
	...
	if (!info->regulator) {
		pr_debug("%s rt5738 register regulator fail\n", __func__);
		return -EINVAL;
	}
